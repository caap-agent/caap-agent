...
KEYS_USED = {
    'button': BASIC_KEYS + ['content'],
    'text': BASIC_KEYS + ['content'],
    'input_field': BASIC_KEYS + ['content'],
    'image': BASIC_KEYS + ['content'],
    ...
    for key in KEYS_USED[type]:
        ...
        if key == 'content':
            if 'content' in element:
                pair_list.append(f'content: "{element["content"]}"')
            else:
                pair_list.append(f'content: ""')
...
def equal_by_text(elt, elt_o):
    if elt['type'] in [1, 2, 3]:
        return elt.get('content') == elt_o.get('content')
    else:
        return True
