...
from src.caap_prompter import CAAP_prompter as CAAP_prompterCls
...
def new_job(self):
    self.ask_main_controller('open_WebShop')()
    logger.info(msg='Webshop opened!')
    ...
    # Save Img
    w_handle = self.ask_main_controller('webshop_window_handle')
    win32gui.SetForegroundWindow(w_handle)
    self.ask_main_controller('reset')
    time.sleep(1)

    # Set mouse initial mouse point for avoid overlap    
    screen_region = self.ask_main_controller('webshop_screen_region')
...
def load_job(self):
    ...
    env_name = self.ask_main_controller("env_name")
    env_name = env_name.replace("-", "_")
    subfolders = [f for f in os.listdir(dir_path) if
                  os.path.isdir(os.path.join(dir_path, f)) and f.startswith(env_name)]
...
def preset_1(self):
    project_folder_name = '20231113_090011_new'
    self.project_folder = os.path.join(self.result_folder, project_folder_name)

    job_folder_name = 'click_scroll_list_2000__20231128_141611'
    self.job_folder = os.path.join(self.project_folder, job_folder_name)
...
def _reinitialize_after_load(self):
    match = re.match(r'^(.+?)_([0-9]+)__', os.path.basename(self.job_folder))
    if match:
...
def _save_remained_actions(self):
    # Read QnA
    action_idx = round(self.selected / 2 - 0.25)
    required, action_queue, contents = caap_prompter_answer_required(self, action_idx)
    if not required:
        # save remained actions
        llm_answer = contents.split(CAAP_prompterCls.llm_end_write_title)[0]
        ...
        with open(os.path.join(self.job_folder, record_filename), 'w', encoding='utf-16') as file:
            ...
            file.write(CAAP_prompterCls.llm_end_write_title)
            file.write(CAAP_prompterCls.answer_write_title)
...
def send_to_caap_prompter(self):
    ...
    CAAP_prompter = self.ask_main_controller("CAAP_prompter")
    CAAP_prompter.job_folder = self.job_folder
    CAAP_prompter.action_idx = action_idx + 1  # next action
    CAAP_prompter.generate_prompt()
...
def _execute(self, action_str):
    ...
    reg_for_type = r'\s*string_to_type\s*:\s*(?P<yy>.+?)\s*\}'
    ...
    elif action in ["press_control_A", "press_control_C", "press_control_V", "press_pageup_button", "press_pagedown_button", "press_home_button"]:
...
def caap_prompter_answer_required(self, action_idx):
    if action_idx <= 0:
...
def get_action_queue(self, action_idx):
    ...
    if self.job_folder is None:
        message = "No project or job selected in the LLM Recorder. Please select a project or job."
    ...
    # Find action
    answer = contents.split(CAAP_prompterCls.answer_write_title)[-1]
...
def _execute__short_cut(self, action_name: str):
    ...
    elif action_name == "press_pageup_button":
        pyautogui.press('pageup')
    elif action_name == "press_pagedown_button":
        pyautogui.press('pagedown')        
    elif action_name == "press_home_button":
...
def _handle_error(self, element_id, element_list, action_name, action_str):
    if action_name is None:
        ...
        error = f"action is not in appropriate form to parse it or is unknown"
...
def update(self):
    ...
    image_process_buttons_list.append(frame.children['image_process'].children['visualobserver_group'].children['!button'])
    image_process_buttons_list.append(
        frame.children['image_process'].children['visualobserver_group'].children['!button2'])
    ...
    if self.selected % 2 == 0:
        ...
        if action_idx < len(self.action_list) - 1 or self.job_status not in ['_PASS', '_FAIL']:
            required, _, _ = caap_prompter_answer_required(self, action_idx)
            if not required:
                # disable "Send to CAAP_prompter" button
...
def _refresh_job_status(self):
    ...
    if len(status_folders) != 1:
        raise Exception("status folder not properly set")
...
def _dump_action_to_json(self):
    json_filename = os.path.join(self.job_folder, "actions.json")
    with open(json_filename, "w", encoding='utf-16') as f:
...
def _capture_screen_wo_cursor(self):
    time.sleep(1.6)  # cursor blinks out after half a second
    screen_region = self.ask_main_controller('webshop_screen_region')
    img = pyautogui.screenshot(region=screen_region)
    return img
...
def _check_done(self):
    WebShop = self.ask_main_controller('WebShop')
    reward = WebShop.get_reward()

    if reward > 0:
        self.job_status = '_PASS'
        self.job_reward = reward
    elif reward == 0:
