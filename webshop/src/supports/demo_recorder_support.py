...
def detect_more_mouse_actions(self, screen_region):
    ...
    while True:
        ...
        if alt_pressed:
            ...
            # Wait until released
            while True:
                released = not win32api.GetKeyState(Alt_Key) < 0
                if released:
                    break
        ...
        # Mouse is released
        released = not win32api.GetKeyState(Left_Key) < 0
        if released:
            break
...
def detect_type_action(self, screen_region):
    ...
    if FLAG__key_pressed:
        ...
        if key in ['pageup', 'pagedown', 'home']:
            control_key_detected(self, screen_region, key)
            return
        ...
        # Wait until released
        while True:
            released = not win32api.GetKeyState(value) < 0
            if released:
                break
...
def control_key_detected(self, screen_region, key):
    ...
    if key == 'A':
        ...
        action['key_combination'] = 'Page_Up'
    elif key == 'pagedown':
        action['key_combination'] = 'Page_Down'
    elif key == 'home':
        action['key_combination'] = 'Home'
