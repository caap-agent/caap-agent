...
def generate_prompt(self):
    ...
    self.use_last_reasoning = self.ask_main_controller("use_last_reasoning")
...
def _make_prompt__direction(self):
    ...
    section_sentence_dict = {5: "five sections", 1: "section", 4: "four sections"}
...
Information for the successful completion
- Good query (re)formulation based on the instruction is important.
- To enter the query in input_field, you must first perform a click.
- Search results appear on a maximum of 5 pages with 10 items each.
- You can enter the product details page by clicking the product ID.
- Product titles in search results show one of the available options, but there may be other available choices on the detail page.
...
- MAKE SURE that when satisfying the task completion conditions, click the 'Buy Now' button to finish.
...
"""    
    ...
    prompt_part1 = f"""\
reiterate the goal of the task in your own language.
"""
...
"""
    ...
    prompt_order = [prompt_part2, prompt_part3, prompt_part4]
