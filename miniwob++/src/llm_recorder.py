...
class LLMRecorder(BaseWindow):
    ...
    def fill_frame__Operations(self, frame):
        ...
        subframe = tk.Frame(img_frame, name='screen2text_group')
        ...
        button = tk.Button(subframe, width=12, height=2, text="Import",
                           command=self.import_screen2text)
        button.pack()
        ...
        button = tk.Button(frame, width=18, height=3, text="Send to\nCAAP Prompter",
                           command=self.button_click__send_to_oracle)
        button.pack()
        button = tk.Button(frame, width=18, height=2, text="ACT",
                           command=self.act_by_oracle_answer)
        button.pack()

        txt_label = tk.Label(frame, text="")  # spacer
        txt_label.pack()

        button = tk.Button(frame, width=26, height=2, text="Act from the Top",
                           command=self.act_from_the_top)
        button.pack()
    ...
    def import_screen2text(self):
        llm_recorder_support.import_screen2text(self)
    ...
    def button_click__send_to_oracle(self):
        self.ask_main_controller("open_CAAP_prompter")()
        logger.info(msg='CAAP Prompter opened!')
        self.send_to_oracle()

    def send_to_oracle(self):
        llm_recorder_support.send_to_oracle(self)

    def act_by_oracle_answer(self):
        llm_recorder_support.act_by_oracle_answer(self)

    def act_from_the_top(self):
        llm_recorder_support.act_from_the_top(self)
